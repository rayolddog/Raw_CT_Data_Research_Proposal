...
**The deployment argument is separate from the information argument, and it does not collapse under the data-processing inequality.** A scanner already ships with the hardware this needs. Reconstruction — iterative, and increasingly deep-learning-based — is itself compute-intensive, so the recon engine carries substantial accelerator capacity, and the projections are *already resident in its memory before reconstruction runs*. A projection-domain detector therefore requires no new hardware, no PACS integration, no data movement outside the scanner, and no raw-data retention policy, because it can score the projections in flight and discard them. This matters because the retention problem of §6.3 is a *training* obstacle, not a deployment one: a deployed raw-data reader never needs to store what it reads. The consequence for the clinical pathway is a **latency advantage that no image-domain tool can match by construction**. An image-domain detector cannot act until reconstruction completes and the series reaches PACS; a projection-domain detector can raise a flag *before the images exist*, at the earliest point in the pipeline at which anything about the study can be known. In the motivating scenario — a trauma-protocol head CT acquired while the reading queue is backed up and the emergency physician has not yet looked — that is not a marginal gain. It also means the two detectors are complementary in **time** as well as in error mode: the raw-domain model yields a provisional flag at acquisition, the image-domain model confirms or contradicts it moments later, and the pair therefore delivers both an earlier alert and a cross-check. Note carefully what this argument does *not* depend on. The subsumption caveat above is a statement about information content and it remains correct; but a detector that runs on hardware already present, before reconstruction, at no integration cost is justified by deployment economics and latency, which the data-processing inequality does not govern. Even in the limit where a raw-domain model offered no accuracy advantage whatever, running it would still buy time no image-domain system can recover.
